...
A rename operation is well-formed whenever the rename function $\pi : |\Gamma| \to \vars$ is an injection, and the codomain is a permutation of a subset of $\vars$ with exactly $k$ elements, where $k$ is the number of declarations in $\Gamma$. 
...
The solution we adopted here is to enable the user to add identity embedding between theories. This allowed us to substitute for morphisms that are not generated in situations like the one we describe here. 
