...
\lstmath{Carrier $\to$ AdditivePointedMagma}, 
\lstmath{AdditiveMagma $\to$ AdditivePointedMagma}, 
and \lstmath{Pointed0 $\to$ AdditivePointedMagma}. 
...
One would want to have \lstmath{AdditiveMonoid} with all the morphisms we introduced in Figure~\ref{fig:cube2_monoid}. In this section we discuss how close we can get to this diagram. 

We have discussed the construction of \lstmath{AdditivePointedMagma}, and showed how the construction is not precisely as in Figure~\ref{fig:cube2_monoid}, although all the morphisms are defined, some are composed of other morphisms. Now, we focus more on the part of defining \lstmath{AdditiveUnital}. The definition of \lstmath{AdditiveLeftUnital} and \lstmath{AdditiveRightUnital} goes as follows 
\begin{lstlisting}
...
\lstmath{AdditiveUnital $\to$ AdditiveMonoid}, 
\lstmath{AdditiveSemigroup $\to$ AdditiveMonoid}, and 
\lstmath{Monoid $\to$ AdditiveMonoid}. 

We thought of the following solutions to this problem 
\begin{itemize}
    \item Computing the colimit of the three theories. 
    \item Dealing with the graph that generates \lstmath{Monoid} using diagram combinators \ednote{cite the CICM2019 paper}. 
\end{itemize}
Our approach is to defined these techniques in terms of pushouts, i.e. provide them as a syntactic sugar to performing multiple pushouts. Since pushouts in the category of contexts are associative, we believed we are able to do that, but the examples above shows that this is not always possible. Therefore, we provided a construction that allows the user to add identity arrows between two theories.\ednote{guarantees that this does not mess with the all-commute-path principle.} 
...
\begin{itemize}
    \item Although the operation we want to know is a rename, we used the \lstmath{combine} operation to perform it. This estanlishes more connections between 
