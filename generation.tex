...
The axiomatic presentation of a class of algebras is defined as $\Gamma = (\sort, \fsyms, \equations)$.  where $\sort$ is a set containing a single sort, $\fsyms$ is a set of operations, and $\equations$ is a set of equations. In the following, we will describe some constructions in terms of the three components of $\Gamma$. Because $\sort$ contains a single sort, we use the same symbol to refer to it, i.e. we use the symbol $\sort$ to refer to the only sort in the set $\sort$. The definitions are adapted from~\cite{ehrig1985fundamentals} and~\cite{handbook1993Maibaum}.  
