...
\begin{itemize}
    ...
    \item SubTheory: An algebra \lstmath{B} is the subalgebra of \lstmath{A} if the following conditions are satisfied %source: ehrig and mehr book 
    \begin{enumerate}
    \item \lstmath{S$_B$} \lstmath{$\subseteq$} \lstmath{S$_{A}$}
    \item \lstmath{c$_B$} \lstmath{=} \lstmath{c$_{A}$} for every constant symbol in the set of function symbols \lstmath{OP}. 
    \item \lstmath{f$_B$(x$_1$ $\cdots$ x$_n$)} \lstmath{=} \lstmath{f$_A$(x$_1$ $\cdots$ x$_n$)}, for all \lstmath{f $\ \in\ $ OP}, {x$_1$ $\cdots$ x$_n$ $\in$ S}, and \lstmath{n $\in$\ $\mathbb{N},\ n\ \geq\ 1$}. 
    \end{enumerate}
    ...
    \item let \lstmath{A} and \lstmath{B} be algebras of the same specification, the \emph{homomorphism} between them is a function \lstmath{hom : A $\to$ B} such that 
    \begin{itemize}
        \item for every constant symbol in \lstmath{OP}: \lstmath{hom c$_A$ = c$_B$} 
        \item for every function symbol in \lstmath{OP}: 
    ...
    \item Quotient Algebras: let \lstmath{A} be a Sig-algebra and \lstmath{R} be a congruent relation on \lstmath{A}. The quotient algebra \lstmath{A/R = (S$_Q$,OP$_Q$)} is defined as
    \begin{itemize}
       \item The carrier elements are sets of equivalence classes \lstmath{S$_Q$ = $\{$[a] $\mid$ a $\in$ S$\}$} where \lstmath{[a] = $\{$b $\mid$ b $\in$ S, (a,b) $\in$ R$\}$}. 
       \item \lstmath{c$_Q$ = [c]}, for every constant symbol \lstmath{c} in \lstmath{A}
       \item A function \lstmath{f x$_1$ $\cdots$ x$_n$} \ednote{check this} 
    \end{itemize}      
    ...
    \item Structural induction: Let \lstmath{p} be a predicate defined on terms \lstmath{t $\in$ T$_{OP}$(X)} of a signature \lstmath{SIG = (S,OP)} with variables \lstmath{X}. The assertion \lstmath{p(t)} is true for all \lstmath{t $\in$ T$_{OP}$} if the following conditions are satisfied: 
    \begin{itemize}
    ...
    \end{itemize}     
    \item Evaluation functions: Given a theory \lstmath{SIG = (S,OP,E)}, a term language defined as above, and an algebra over a carrier \lstmath{A}. The function \lstmath{eval : T$_{OP}$ $\to$ A} is defined recursively as 
    \begin{itemize}
...
\end{itemize}\ednote{Is there more that can be added here?} 
...
\lstmath{eqInstance :: EqTheory -> Maybe Int -> EqInstance} 
