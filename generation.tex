...
The axiomatic presentation of a class of algebras is defined as \lstmath{Th = (S,F,E}) where \lstmath{S} is a single sort, \lstmath{F} is a set of operations, and \lstmath{E} is a set of equations.  In the following, we will describe some constructions in terms of the three components of \lstmath{Th}. The definition are adapted from~\cite{ehrig1985fundamentals} and~\cite{handbook1993Maibaum}.  

\begin{itemize}
    \item The \emph{signature} of a theory \lstmath{(S,F,E)}  is \lstmath{Sig = (S,F)} consisting of the sort and $n$-ary function symbols, where $n \geq 0$. The signature specifies the language of the theory, without any laws. 
    \item The \emph{product} of two algebras \lstmath{A} and \lstmath{B} of the same theory \lstmath{Th} is the algebra with sort \lstmath{(S$_A$ $\times$ S$_B$)}. In a uni-sorted setup, the sort of the product algebra is \lstmath{(S$\times$S)}. 
    \begin{itemize}
    \item \lstmath{c$_\times$ : (S$_A$ $\ \times\ $ S$_B$)}, for every constant symbol \lstmath{c} in \lstmath{Th}. 
    \item \lstmath{op$_\times$ : (S$_A$ $\ \times\ $ S$_B$) $\ \to$ $\ \cdots\ $  $\to\ $ (S$_A$ $\ \times\ $ S$_B$)}, for every function symbol \lstmath{op} in \lstmath{Th} based on its arity. 
    \item The set of equations \lstmath{E$_\times$} is given by substituting the new sort, constant and function symbols in the set \lstmath{E}.  
    \end{itemize}
    \item The \emph{sub-theory} \lstmath{B} of a theory \lstmath{A} is defined as 
    \begin{enumerate}
    \item \lstmath{S$_B$} \lstmath{$\subseteq$} \lstmath{S$_{A}$}
    \item \lstmath{c$_B$} \lstmath{=} \lstmath{c$_{A}$} for every constant symbol in the set of function symbols \lstmath{F}. 
    \item \lstmath{f$_B\ $ x$_1\ $ $\ \cdots\ $ x$_n$} \lstmath{=} \lstmath{f$_A\ $x$_1\ $ $\ \cdots\ $ x$_n$}, for all \lstmath{f $\ \in\ $ F}, {x$_1$ $\cdots$ x$_n$ $\in$ S}, and \lstmath{n $\ \in\ $ $\mathbb{N}$, n $\ \geq\ $ 1}. 
    \end{enumerate}
    ...
    \item The \emph{homomorphism} between two algebras \lstmath{A} and \lstmath{B} of the same theory \lstmath{Th} is a function \lstmath{hom : S$_A$ $\ \to\ $ S$_B$} such that 
    \begin{itemize}
        \item for every constant symbol in \lstmath{F}: \lstmath{hom c$_A$ = c$_B$} 
        \item for every function symbol in \lstmath{F}: 
           \lstmath{hom(f$_A\ $ x$_1\ $ $\ \cdots\ $ x$_n$) = f$_B\ $ (hom x$_1$)  $\ \cdots\ $ (hom x$_n$)} 
    \end{itemize}
    ...
    \end{itemize}
    \item The \emph{kernel} of a homomorphism from algebra \lstmath{A} to algebra \lstmath{B} is defined as the binary relation \lstmath{$\equiv_{hom}$} on the sort of \lstmath{A}, such that 
    \begin{lstlisting}[mathescape] 
    a $\equiv_{hom}$ b $\Leftrightarrow$ hom a $\equiv_{hom}$ hom b
    \end{lstlisting}
    for every \lstmath{a} and \lstmath{b} in \lstmath{S$_A$}. 
 ...
 %   \item Homomorphism Equality 
    \item The \emph{relational interpretation} between two algebras \lstmath{A} and \lstmath{B} of the same theory is a relation \lstmath{interp a b}, for all \lstmath{a $\ \in\ $ S$_A$} and \lstmath{b $\ \in\ $ S$_B$} such that 
    \begin{itemize}
    \item interp c$_A$ c$_B$, for every constant \lstmath{c $\ \in\ $ A}.  
    \item interp x$_1$ y$_1$ $\ \wedge\ $ $\ \cdots\ $ $\ \wedge\ $ interp x$_n$ y$_n$ $\Rightarrow$ interp (op$_A$ x$_1$ $\ \cdots\ $ x$_n$) (op$_B$ y$_1$ $\ \cdots\ $ y$_n$) for all function symbols \lstmath{op $\ \in\ $ F}, where \lstmath{x$_1$ $\ \cdots\ $ x$_n$ $\ \in\ $ S$_A$} and \lstmath{y$_1$ $\ \cdots\ $ y$_n$ $\ \in\ $ S$_B$}. 
    \end{itemize}
    
    \item The \emph{quotient algebra} for a theory \lstmath{Th} with respect to some congruence relation \lstmath{$\equiv$} is defined as the theory \lstmath{Th/$\equiv\ $ = (S$_{Q}$,F$_{Q}$,E$_{Q}$)} such that\ednote{Q: How to define \lstmath{E$_{Q}$}} 
    \begin{itemize}
       \item \lstmath{S$_Q$} is the factor set of \lstmath{S}, defined as 
       	\begin{lstlisting}[mathescape]
       	S$_Q$ = {[x] $\mid$ x $\in$ S}
       	\end{lstlisting} 
       	where \lstmath{[x]} is the equivalence class defined as \lstmath{[x] = $\{$y $\ \in\ $ S $\ \mid\ $ x $\ \equiv\ $ y$\}$}
       \item \lstmath{c$_Q$ = [c]}, for constant symbols \lstmath{c $\ \in\ $ F} and \lstmath{c$_Q$ $\ \in\ $ F$_Q$}.  
       \item \lstmath{f$_Q$ [x$_1$] $\ \cdots\ $ [x$_n$]} = [f x$_1$ $\ \cdots\ $ x$_n$]
       for function symbols \lstmath{f$_Q$ $\ \in\ $ F$_Q$} and \lstmath{f $\ \in\ $ F}.
    \end{itemize}      
...
%    \item The monotonicity of a function 
    ...
    \begin{itemize}
        ...
        \item A term \lstmath{t$_f$ $\ $ t$_1$ $\ \cdots\ $ t$_n$} for every function symbol \lstmath{f : s $\ \to\ $ $\ \cdots\ $ $\ \to\ $ s} of arity $n$, such that  \lstmath{t$_1$ $\ \cdots\ $ t$_n$} belongs to the language. 
    \end{itemize}
    ...
    \item The \emph{staged term language} of a theory is the term language in which expressions can be marked for execution in compile or runtime stages \ednote{Q: Can I find any better decisions}.  
    \item Structural induction: Let \lstmath{p} be a predicate defined on terms \lstmath{t $\ \in\ $ T$_{F}$(X)} of a signature \lstmath{SIG = (S,F)} with variables \lstmath{X}. The assertion \lstmath{p(t)} is true for all \lstmath{t $\ \in\ $ T$_{F}$} if the following conditions are satisfied: 
    \begin{itemize}
        \item \lstmath{p(t)} is true for all constant and variable symbols 
        \item for each term \lstmath{f t$_1$ $\ \cdots\ $ t$_n$}, we have that if \lstmath{p(t$_1$) $\cdots$ p(t$_n$)} are true then also \lstmath{p(f t$_1$ $\ \cdots\ $ t$_n$)} is true.  
    \end{itemize}     
    \item Evaluation functions: Given a theory \lstmath{SIG = (S,F,E)}, a term language defined as above, and an algebra over a carrier \lstmath{A}. The function \lstmath{eval : T$_{F}$ $\ \to\ $ A} is defined recursively as 
    \begin{itemize}
        \item \lstmath{eval(c) = c$_{A}$}
        \item \lstmath{eval(f t$_1$ $\ \cdots\ $ t$_n$)} = f$_A$ (eval t$_1$) $\cdots$ (eval t$_n$) where an assignment function maps the constants of the language to the constants defined by the algebra. The evaluation function for open term language would be similar except it has an additional environment that assigns value of the carrier to variables. 
    \end{itemize}      
...
The input to the generator is a list of tog dependent records. The first step the generator needs to do is to cast them into instances of \lstmath{EqTheory}. 
\begin{comment}
...
Instances of a theory would also contain fields corresponding to those in the theory. Instead of having this as part of the type, we provide functions \lstmath{projectConstr} and \lstmath{applyProjConstr} to compute them. 
When projecting a member from a record, be it a representation of a theory or an instance, one might want to have the projected member as a type declaration or as a term in the language. The function \lstmath{projectConstr} provides the projected member type declaration, while \lstmath{applyProjConstr} provides it as a term. 
The \lstmath{projectConstr} function checks if the member being projects is an argument. If so, then its name of the binding list is used to construct the expresion. If not, then it needs to be qualified by the instance name. 
\begin{hscode} 
...
Our generation algorithms manipulates function symbols in different ways. It is essential to provide the key tools for this manipulation. A function symbol as a member of a theory has the type \lstmath{Constr}. It gives rise to terms in the language of the theory based on its arity. The arguments to the terms are bindings, and that's why we define the type \lstmath{FApp} of function application to variables, as follows 
\begin{hscode}
type FType = Constr
...
For a theory \lstmath{(S,F,E)}, the signature is obtained by dropping the list of axioms \lstmath{E}. In tog, the process of generating the signature is done in $3$ steps via the \lstmath{signature_} function. 
\begin{hscode}
signature_ :: Eq.EqTheory -> Eq.EqTheory
signature_ = over Eq.thyName (++ "Sig") . set Eq.axioms [] .  gmap ren
\end{hscode}
First, one need to rename the fields of the theory, to be able to reuse them in the signature definition. This is because the tog scope checker would not accept overloaded names of fields within the same module. The \lstmath{gmap ren} part of the function definition traverses the \lstmath{EqTheory} renaming fields by adding a suffix \lstmath{S} to each of them. In case the code is exported into a system that support this kind of overloading, the suffix can be removed. Note that this rename will be needed when generating any new construction. 
...
Using this framework we are able to generate a library of $106468$ lines of code from the representation of $230$ theories. Appendix~\ednote{add the appendix and reference} shows the generated definition for \lstmath{Monoid} theory. All the generated files are present on github under~\url{https://github.com/ysharoda/tog/blob/master/Library/mathscheme-generated.tog}~\ednote{put theories in separate files like in agda output, and update the url}. 
