...
Despite the many efforts dedicated to building math libraries\ednote{some references}, a UDML has not become a reality. One reason is that developing and maintaining libraries of mathematics requires a lot of manpower. One would want to believe that this human effort is put into the creative work of formalizing new pieces of knowledge. By examining current libraries of theorem provers, we know this is not always true. The process of creating libraries for theorem provers involve a lot of redundancy. We give examples from libraries of formal systems in Chapter~\ref{ch:redundancy}. To understand why this redundancy exists, we examine the formalization of \lstmath{Monoid} in algebra textbooks versus in theorem provers. 

\lstmath{Monoid} is an algebraic structure that describes algebras with carrier set and associative binary operation with a unit over that set. The definition given in~\cite{jacobson1985basic}  is 
\begin{quote}
...
\end{quote}
The definition of \lstmath{Monoid} is followed by the definition of monoid homomorphism as
\begin{quote}
...
If we try to capture these definitions with a minimal (imaginary) computer language we would have the following definitions 
...
\end{lstlisting}
Notice how the two definitions of homomorphisms are similar. The homomorphism of a theory \lstmath{X} is a mapping between two instances of \lstmath{X}, given by those $3$ components 1) the $2$ instances of the theory, 2) the function \lstmath{hom} that maps the carriers of the $2$ instances, and 3) preservation axioms \lstmath{pres-x} for every operation symbol \lstmath{x}. The preservation axioms follow the pattern
\begin{lstlisting}[mathescape]
hom (op$_1$ x$_1$ .. x$_n$) = op$_2$ (hom x$_1$) .. (hom x$_n$)
\end{lstlisting}
Our observation of this pattern gives rise to some questions 
\begin{itemize}
\item[Q1] Is this pattern specific to \lstmath{Monoid}, \lstmath{Group}, and their homomorphisms? 
\end{itemize}    
\lstmath{Monoid} and \lstmath{Group} are two of many algebraic structures used in abstract algebra. The commonalities between these structures is the subject of study of universal algebra. A theory\ednote{this is the first occurence of the word theory, need to bridge into it} in universal algebra is defined as~\cite{mckenzie1987algebras}
\begin{quote}
...
\end{quote}\ednote{look for a text book that explicitly state axioms. The properties of the function symbols, which are mentioned in mathematics textbooks but not always stated, are stated as axioms in formal systems, and so we defined an algebraic theory to be a type ($\langle A, F \rangle$,$E$), where $E$ is the set of axioms. }
For every theory that fit in this definition, we have a uniform definition of homomorphism, subalgebra, quotient algebra, term language and others. \lstmath{Monoid} theory is one of those captured by this abstraction. We have given a definition of \lstmath{Monoid} in an imaginary language that fits in this abstraction. In Figure~\ref{fig:mon-diff-lang}, we show the definitions of \lstmath{Monoid} in $5$ different language. 
\begin{figure}
...
\begin{itemize}
    \item[Q2] Is there enough information that can be generated? 
\end{itemize}
...
\begin{itemize}
    \item[Q3] What meta theory is used? 
\end{itemize}
...
We support the building of this library by providing combinators to define the library theories and algorithms to compute related construction.rs
