...
\end{lstlisting}
These definition overlook that in some cases one might want to talk about an operation with only right unit, like in the case of subtraction on integers. One will then need to add a new theory that is similar to \lstmath{Unital} without the \lstmath{lunit} declaration. Any theorems proved in the context of \lstmath{Unital} cannot be used, even if it only depends on \lstmath{lunit}. \ednote{Another motivating example will be given when talking about building pointedMagma with combinators. remember to refer to this here}.  
