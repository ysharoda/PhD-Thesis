...
refer to a pair that contains the value of \lstmath{n : $\;\mathbb{N}$} in the first position and a vector of length \lstmath{n} in the second one. 
...
\Delta_1 \arrow["v_1",rr] \arrow["w_1",rrrd, bend right]& & \Phi \arrow[dashed, rd] \\ 
& & & \Omega
\end{tikzcd}} 
...
\section{Relational Interpretation}\ednote{add this section to the description} 
A program consisting of a type and operations on that type can be seen as a theory. 
When proving properties about programs, one often want to talk about structure-preserving relations between them, like equivalence. These are called relational interpretations and are seen as a generalization of homomorphisms~\cite{reynolds1983types}. Relational interpretations are used extensively when working on the semantics of programming languages. They form the bases of a proof technique called \emph{logical relations}~\cite{crary2005logical}. Supporting proofs by logical relations has been considered a benchmark for theorem provers in the revised POPLmark challenge~\cite{poplmark2019}.  

We give the definition of relational interpretations in Section~\ref{sec:toBeGenerated}. 
