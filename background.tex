...
where the type \lstmath{M} depends on the \emph{value} of \lstmath{x}, i.e. \lstmath{M : A $\ \to\ $ Type}. 
% source: Type theory and formal proof, P. 103 - Ch.5 
Having types that depends on values adds to the expressiveness of the logic. A common example for introducing dependent types is the type of a vector of \lstmath{n} elements of a type \lstmath{A}. 
...
A dependent pair, in which later elements depends on values of earlier ones, are referred to as $\Sigma$-types. The type 
