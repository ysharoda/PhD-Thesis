...
where the type $M$ depends on the \emph{value} of $x$, i.e. \lstmath{$M : A \;\to\; \text{Type}$}. 
% source: Type theory and formal proof, P. 103 - Ch.5 
Having types that depends on values adds to the expressiveness of the logic. A common example for introducing dependent types is the type of a vector of $n$ elements of a type $A$. 
In most programming languages, the type of this vector is defined in terms of the type of its elements as $Vec\;A$. Using dependent types, the type of a vector can depend on both the type of its elements and also its length, written as \lstmath{$\Pi\; n : \mathbb{N}\;\cdot\; Vec\;A\;n$}.
...
\caption{Grammar for first order dependent type language with dependent sum types. Adapted from~\cite{pierce2005advanced}.}
...
A dependent pair, in which later elements depends on values of earlier ones, are referred to as $\Sigma$-types. The type \lstmath{$\Sigma\;[n : \mathbb{N},\;Vec\;A\;n]$} refer to a pair that contains the value of \lstmath{n} in the first position and a vector of length \lstmath{n} in the second one. 
...
\lstmath{$[A : \text{Type}][n : \mathbb{N}][Vec\;A\;n]$}. 
...
$\Sigma$ [A : Type, $\Sigma$ [op : A $\;\to\;$ A $\;\to\;$ A, $\Sigma$ [e : A, 
...
Using little theories increases the reuse ability of results. For example, if the theorem \lstmath{op e e = e} is proven in the theory \lstmath{Unital}, it can be transported to all theories that are connected to \lstmath{Unital} via morphisms, like \lstmath{Monoid}. On the other hand, if it is proven in the theory \lstmath{Group}, it cannot be transported to \lstmath{Monoid}, because all declarations in \lstmath{Group} becomes part of the context for proving the theorem.  
