...
\begin{itemize}
\item $\sort$ is a set containing one sort \lstmath{s}.\footnote{In case of multi-sorted logics, the set $\sort$ is an indexed set of sorts. We restrict ourselves to unisorted logics as it captures the algebraic structures we're interested in, like monoids, groups, and rings.}
...
The set $\axioms$ is closed under logical consequence and is infinite in almost all cases. 
...
Theory graphs leverage the structure of mathematics by relying on morphisms to connect them. Therefore, we are able to decompose the development of theories. Having morphisms also facilitates the transportation of results between theories. 
Here we discuss two techniques for decomposing theories; little and tiny theories.  
