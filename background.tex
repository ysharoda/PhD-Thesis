...
DTT is seen by many as a convenient foundation for representing mathematics~\cite{whyDttCoq, whyDTTbauer, nCatShulman}\ednote{Q: am I citing them the write way?}. The powerful type system allows for expressing ideas frequentally used in mathematics. Statements like "the non-zero element", operations like projecting the first unit vector in a specific dimension, or representation of a family of sets. 
...
Figures~\ref{fig:dttGrammar} and~\ref{fig:dtt-w-sigma} shows the grammar and typing rules of a small version of dependent type theory with $\Sigma$-types like the one we use in this work. The terms permissible in this type theory are variables, $\lambda$-abstraction, function application, dependently types pair, and its projections. In additions to types and type applications, the DTT we present here allows $\Pi$- and $\Sigma$-types. The concepts of $\Sigma$-types and contexts will be discussed shortly in this section.
